function data = simulate_phase1c_case07(cfg,signals,noise)
%SIMULATE_PHASE1C_CASE07 Run one independent Case07 branch on AutoComp9.

requiredNoise = {'A','B','C'};
for k = 1:numel(requiredNoise)
    name = requiredNoise{k};
    if ~isfield(noise,name) || numel(noise.(name)) ~= numel(signals.t)
        error('Phase1C:Step5Noise','Invalid shared noise vector %s.',name);
    end
end
...
data.Cb = data.Ca;
data.Cc = data.Ca;
data.scenario = 'Case07_overlap_drift_fault';
...
if max(abs(data.Cs1-signals.Cs1_pF)) > 1e-10 || ...
    ...
    error('Phase1C:Step5ModelTruth', ...
        'Simulink Case07 Cs outputs do not match the frozen input truth.');
end
end
